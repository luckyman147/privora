...
export function WelcomeScreen({ d, form, onStart }: Props) {
  ...
  const content = (
    <>
      <div style={{ alignSelf: layout === 'center' ? 'center' : 'flex-start', marginBottom: 8 }}>
        {renderLogo(d, 'welcome', d.primary_color)}
      </div>
      <h1 style={{ fontSize: 26, fontWeight: 700, fontFamily: d.heading_font, color: textColor, marginBottom: 12 }}>{title}</h1>
      {subtitle && <p style={{ color: textColor + 'cc', fontSize: 15, lineHeight: 1.5, marginBottom: 16 }}>{subtitle}</p>}
      {d.welcome_content && <p style={{ color: '#64748b', fontSize: 14, lineHeight: 1.6, marginBottom: 24, whiteSpace: 'pre-wrap' }}>{d.welcome_content}</p>}
      {d.welcome_container_content && <p style={{ color: '#475569', fontSize: 13, lineHeight: 1.5, marginBottom: 20 }}>{d.welcome_container_content}</p>}
      <button onClick={onStart} style={{ background: btnColor, color: '#fff', border: 'none', borderRadius: btnR, padding: btnPad, fontSize: 16, fontWeight: 600, cursor: 'pointer', alignSelf: layout === 'left' ? 'flex-start' : 'center' }}>
